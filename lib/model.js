...
_.extend(Model, {
    ...
    find: function (options, callback) {
        ...
        var self = this,
            query = {
                ...
                HashKeyValue: {
                    S: _.isArray(options.hash) ? options.hash.join(Schema.keyDelimiter) : options.hash
                },
        ...
        if (options.range)
        {
            query.RangeKeyValue = _.isArray(options.range) ? options.range.join(Schema.keyDelimiter) : options.range
        }
    ...
    findOne: function (options, callback) {
        ...
        if (options.range)
        {
            query.Key.RangeKeyElement = _.isArray(options.range)
                ? options.range.join(Schema.keyDelimiter)
                : Schema.transform.dateAttribute(options.range);
        }
