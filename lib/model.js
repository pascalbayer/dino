...
module.exports = exports = function (options) {
    ...
    var Model = {
        ...
        findOne: function (options, callback) {
            ...
            query.Key[self.schema.hashKey] = self.schema.serializeHashAttribute(options.hash);
            
            if (options.range)
            {
                query.Key[self.schema.rangeKey] = self.schema.serializeRangeAttribute(options.range);
            }
            
            self.connection.client.getItem(query, function(err, data){
                if (err) return callback(err);
                if (!data.Item) return callback(new Error('Item does not exist'));
                return callback(null, self.parse(data.Item), data.ConsumedCapacity.CapacityUnits);
            });
        },
