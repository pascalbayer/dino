...
_.extend(Model.prototype, {
    ...
    set: function (key, val) {
        ...
        for (attr in attrs)
        {
            this.attributes[attr] = !_.isUndefined(attrs[attr]) ? attrs[attr] : schema.get(attr).defaultValue;
        }
    ...
    getHashAttribute: function () {
        ...
        return _.map(schema.hashKeyAttributes, function(attr){
            return schema.attributes[attr].transformValue(self.get(attr));
    ...
    toDynamo: function () {
        ...
        if (schema.hashKeyAttributes.length > 1)
        {
            ...
            dynamoAttrs[schema.hashKey] = new types.String().transform(self.getHashAttribute());
        }
            ...
            && schema.rangeKeyAttributes.length > 1)
        {
            ...
            dynamoAttrs[schema.rangeKey] = new types.String().transform(self.getRangeAttribute());
        }
    ...
    save: function (callback) {
        ...
        var self = this,
            client = self.constructor.client,
            schema = self.constructor.schema;
    ...
    destroy: function (callback) {
        ...
        var self = this,
            ...
            schema = self.constructor.schema,
            key = {
                HashKeyElement: { S: self.getHashAttribute() }
            };
        
        if (schema.rangeKey)
        {
            key.RangeKeyElement = { S: self.getRangeAttribute() };
        }
